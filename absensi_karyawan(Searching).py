...
# sorting dengan menggunakan teknik merge sort
    ...
    # melakukan pencarian dengan jenis pencarian fibonacci
    def cari_nama(self, nama_karyawan):
        current = self.head
        if not current:
            print("Belum ada data kehadiran yang dicatat.")
            input("Tekan Enter untuk melanjutkan...")
        ...
        while current:
            data.append(current)
            current = current.next
        # Urutkan data terlebih dahulu berdasarkan nama karyawan
        sorted_data = self.merge_sort(data, sort_by='nama', ascending=True)

        # Lakukan pencarian menggunakan algoritma Fibonacci
        fib2 = 0
        fib1 = 1
        fib = fib1 + fib2

        while fib < len(sorted_data):
            fib2 = fib1
            fib1 = fib
            fib = fib1 + fib2
        ...
        while fib > 1:
            i = min(offset + fib2, len(sorted_data) - 1)

            if sorted_data[i].nama_karyawan == nama_karyawan:
                print(f"Karyawan '{nama_karyawan}' ditemukan pada indeks ke-{i}.")
                input("Tekan Enter untuk kembali...")
                return

            elif sorted_data[i].nama_karyawan < nama_karyawan:
                fib = fib1
                fib1 = fib2
                fib2 = fib - fib1
                offset = i
        ...
        if fib1 and offset + 1 < len(sorted_data) and sorted_data[offset + 1].nama_karyawan == nama_karyawan:
            print(f"Karyawan '{nama_karyawan}' ditemukan pada indeks ke-{offset + 1}.")
        else:
            print(f"Karyawan '{nama_karyawan}' tidak ditemukan dalam data kehadiran.")
        input("Tekan Enter untuk kembali...")

    def cari_waktu(self, waktu):
        current = self.head
        if not current:
            print("Belum ada data kehadiran yang dicatat.")
            input("Tekan Enter untuk melanjutkan...")
        ...
        while current:
            data.append(current)
            current = current.next
        sorted_data = self.merge_sort(data, sort_by='waktu', ascending=True)
        ...
        while fib > 1:
            ...
            if sorted_data[i].waktu == waktu:
                print(f"Karyawan dengan kehadiran pada pukul {waktu} ditemukan pada indeks ke-{i}.")
                input("Tekan Enter untuk kembali...")
                return

            elif sorted_data[i].waktu < waktu:
                fib = fib1
                fib1 = fib2
                fib2 = fib - fib1
                offset = i
        ...
        if fib1 and offset + 1 < len(sorted_data) and sorted_data[offset + 1].waktu == waktu:
            print(f"\nKaryawan bernama '{nama_karyawan}' dengan kehadiran pada pukul ({waktu}) ditemukan pada indeks ke-{offset + 1}.")
        else:
            print(f"Data dengan waktu '{waktu}' tidak ditemukan dalam data kehadiran.")
        input("Tekan Enter untuk kembali...")
...
while True:
    ...
    if pilihan == '1':
        ...
        print("\nIngin mencari berdasarkan apa ?")
        print("1. Berdasarkan nama karyawan.")
        print("2. Berdasarkan waktu kehadiran.")
        opsi = input("Pilih metode (1/2): ")
        if opsi == '1':
            nama_karyawan = input("Masukkan nama karyawan yang ingin dicari: ")
            pendataan.cari_nama(nama_karyawan)
        elif opsi == '2':
            waktu = input("Masukkan waktu kehadiran yang ingin dicari (format: JJ:MM:DD): ")
            pendataan.cari_waktu(waktu)
        else:
            print("Input tidak valid. Silakan pilih 1 atau 2.")
    elif pilihan == '7':
        print("┌───────────────────────────────────────────────┐")
